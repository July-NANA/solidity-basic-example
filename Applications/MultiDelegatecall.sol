...
contract Helper {
    ...
    TestMultiDelegatecall[]  tests;
    bytes[]  datas;
    address[] testAddrs;
    TestMultiDelegatecall _test;
    function test() public returns(uint[] memory){
        
        for (uint i=0;i<10;++i){
            TestMultiDelegatecall newTest= new TestMultiDelegatecall();
            tests.push(newTest);
            datas.push(this.getFunc1Data(0,i));
            testAddrs.push(address(newTest));
        }
        bytes[] memory byteDatas=callMultiCall(datas);
    ...
    function callMultiCall( bytes[] memory data)
        internal
        returns (bytes[] memory)
    {
        // 转换 memory 到 calldata
        return _test.multiDelegatecall(data);
    }
}
