...
contract TimeLock{
    ...
    uint256 public constant GRACE_PERIOD = 1000; // seconds，执行交易的宽限期

    address public owner;
    // tx id => queued,记录任务是否已存在
    ...
    function cancel(bytes32 _txId) external onlyOwner {
     ...
     function getBytes32() public pure returns(bytes32 num){
        num=bytes32(uint256(1));
     }
}